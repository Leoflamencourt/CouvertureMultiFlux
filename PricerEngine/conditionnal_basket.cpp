...
double ConditionnalBasket::payoff(PnlMat *path, double interestRate) {
    ...
    for (int m = 1; m < nb_payoffs; m++) {
        double t_m = GET(paymentDates, m-1);
        ...
        double K_m = GET(strikes, m-1);

        double P_m = (average_price - K_m) > 0 ? (average_price - K_m) : 0;

        if (P_m > 0) {
            return P_m * exp(interestRate * (GET(paymentDates, paymentDates->size - 1) - GET(paymentDates, m-1)));
        }
    }
    return 0.0;
}
